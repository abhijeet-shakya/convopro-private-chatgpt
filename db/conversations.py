...
conversations.create_index([("last_interacted", DESCENDING)]) # type: ignore

# ---- helpers -----
def now_utc():
    return datetime.now(timezone.utc)

def create_new_conversations_id() -> str:
    return str(uuid.uuid4())

# ---- core services ----
def create_new_conversation(title: Optional[str] = None, role: Optional[str]=None, content: Optional[str] = None) -> str:
    conv_id = create_new_conversations_id()
    ts = now_utc()
    doc = {
        "_id": conv_id,
        "title": title or "Untitled Conversations",
        "messages":[],
        "last_interacted": ts,
    }
